...
test.describe("Intent-driven flow for intent-poc-app", () => {
  test("Verify button stability after intent submission", async ({ page }) => {
    await test.step("Intent Studio Prompt Run", async () => {
      ...
      const screenshotPath = path.join(screenshotRoot, "bdd/intent-poc-app-verify-button-stability-after-intent-submission", "shot-1-verify-button-stability-after-intent-submission-intent-studio.png");
    ...
    await test.step("Configuration Section Visible", async () => {
      ...
      const screenshotPath = path.join(screenshotRoot, "bdd/intent-poc-app-verify-button-stability-after-intent-submission", "shot-2-verify-button-stability-after-intent-submission-configuration-visible.png");
      await mkdir(path.dirname(screenshotPath), { recursive: true });
      await page.screenshot({ path: screenshotPath, fullPage: true });
    });
  });
