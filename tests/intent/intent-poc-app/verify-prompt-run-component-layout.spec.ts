...
test.describe("Intent-driven flow for intent-poc-app", () => {
  test("Verify prompt run component layout", async ({ page }) => {
    await test.step("Intent Studio Prompt Run", async () => {
      ...
      const screenshotPath = path.join(screenshotRoot, "bdd/intent-poc-app-verify-prompt-run-component-layout-spec", "shot-1-verify-prompt-run-component-layout-intent-studio.png");
    ...
    await test.step("Configuration Section Visible", async () => {
      ...
      const screenshotPath = path.join(screenshotRoot, "bdd/intent-poc-app-verify-prompt-run-component-layout-spec", "shot-2-verify-prompt-run-component-layout-configuration-visible.png");
      await mkdir(path.dirname(screenshotPath), { recursive: true });
      await page.screenshot({ path: screenshotPath, fullPage: true });
    });
  });
