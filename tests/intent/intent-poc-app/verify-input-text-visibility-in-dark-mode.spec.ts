...
test.describe("Intent-driven flow for intent-poc-app", () => {
  test("Verify input text visibility in dark mode", async ({ page }) => {
    await test.step("Input Field Ready For Text Entry", async () => {
      ...
      const screenshotPath = path.join(screenshotRoot, "bdd/verify-input-text-visibility-in-dark-mode", "input-field-ready-for-text-entry.png");
    ...
    await test.step("Typed Input Text Visible", async () => {
      ...
      const screenshotPath = path.join(screenshotRoot, "bdd/verify-input-text-visibility-in-dark-mode", "typed-input-text-visible.png");
      await mkdir(path.dirname(screenshotPath), { recursive: true });
