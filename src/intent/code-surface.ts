...
export function inferCodeSurface(input: {
  ...
  if (isIntentPocAppSource(input.primarySourceId) && includesAnyPhrase(normalizedPrompt, INTENT_STUDIO_UI_KEYWORDS)) {
    return {
      ...
      confidence: "high",
      rationale: `The prompt describes Intent Studio form or configuration controls inside ${input.primarySourceId}.`,
      alternatives: [
  ...
  if (isIntentPocAppSource(input.primarySourceId) && includesAnyPhrase(normalizedPrompt, INTENT_STUDIO_RESULTS_KEYWORDS)) {
    return {
      ...
      label: getCodeSurfaceLabel("intent-studio"),
      confidence: "high",
      rationale: `The prompt targets Intent Studio run results, run status, or capture links inside ${input.primarySourceId}.`,
      alternatives: [
